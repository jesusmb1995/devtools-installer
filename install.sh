...
replace__0_v0() {
    ...
    if [ "$(( $([ "_${EXEC_SHELL}" != "_ksh" ]; echo $?) || $(( $([ "_${EXEC_SHELL}" != "_bash" ]; echo $?) && comp )) ))" != 0 ]; then
        result_1006="${source_1003//"${search_1004}"/"${replace_1005}"}"
        __status=$?
    else
        result_1006="${source_1003//"${search_1004}"/${replace_1005}}"
        __status=$?
    fi
    ret_replace0_v0="${result_1006}"
    return 0
}
...
replace_regex__3_v0() {
    local source_998="${1}"
    local search_999="${2}"
    local replace_text_1000="${3}"
    local extended_1001="${4}"
    sed_version__2_v0 
    local sed_version_1002="${ret_sed_version2_v0}"
    replace__0_v0 "${search_999}" "/" "\\/"
    search_999="${ret_replace0_v0}"
    replace__0_v0 "${replace_text_1000}" "/" "\\/"
    replace_text_1000="${ret_replace0_v0}"
    if [ "$(( $(( sed_version_1002 == __SED_VERSION_GNU_1 )) || $(( sed_version_1002 == __SED_VERSION_BUSYBOX_2 )) ))" != 0 ]; then
        # '\b' is supported but not in POSIX standards. Disable it
        replace__0_v0 "${search_999}" "\\b" "\\\\b"
        search_999="${ret_replace0_v0}"
    fi
    if [ "${extended_1001}" != 0 ]; then
        # GNU sed versions 4.0 through 4.2 support extended regex syntax,
        # but only via the "-r" option
        if [ "$(( sed_version_1002 == __SED_VERSION_GNU_1 ))" != 0 ]; then
            local command_1
            command_1="$(sed -r -e "s/${search_999}/${replace_text_1000}/g" <<<"${source_998}")"
            __status=$?
            ret_replace_regex3_v0="${command_1}"
            return 0
        else
            local command_2
            command_2="$(sed -E -e "s/${search_999}/${replace_text_1000}/g" <<<"${source_998}")"
            __status=$?
        ...
        local command_3
        command_3="$(sed -e "s/${search_999}/${replace_text_1000}/g" <<<"${source_998}")"
        __status=$?
...
prompt_user__171_v0() {
    ...
    __status=$?
    local command_13
    command_13="$(dd bs=1 count=100 status=none < /dev/tty 2>/dev/null | tr -d '
')"
    __status=$?
    if [ "${__status}" != 0 ]; then
        printf "
...
url_status__309_v0() {
    local url_1008="${1}"
    local command_38
    command_38="$(curl -sIL ${url_1008} | head -1 | tr -s ' ' | cut -d' ' -f2)"
    __status=$?
    if [ "${__status}" != 0 ]; then
...
extract_extension_id__350_v0() {
    ...
    if [ "$([ "_${ret_replace_regex3_v0__6_8}" != "_${url_997}" ]; echo $?)" != 0 ]; then
...
install_chrome_extension__351_v0() {
    ...
    if [ "$([ "_${http_code_1009}" == "_200" ]; echo $?)" != 0 ]; then
        echo_error__142_v0 "extension url returned HTTP ${http_code_1009} (expected 200): ${url_996}" 1
    fi
    local dir_1010="/etc/opt/chrome/policies/managed"
    mkdir -p ${dir_1010}
    __status=$?
    if [ "${__status}" != 0 ]; then
        ret_install_chrome_extension351_v0=''
...
cmd_update__511_v0() {
    ...
    if [ "$([ "_${old_lines_1309}" == "_" ]; echo $?)" != 0 ]; then
        ...
        for row_1317 in "${rows_1316[@]}"; do
            ...
            if [ "$(( ${#__length_77[@]} >= 2 ))" != 0 ]; then
                old_keys_1310+=("${parts_1318[0]?"Index out of bounds (at /tmp/jbtd-install-build-IHWX8r/install.ab:485:36)"}")
                old_vals_1311+=("${parts_1318[1]?"Index out of bounds (at /tmp/jbtd-install-build-IHWX8r/install.ab:486:36)"}")
            fi
        done
    fi
    local changed_1319=()
    ...
    for row_1321 in "${new_rows_1320[@]}"; do
        ...
        local key_1323="${parts_1322[0]?"Index out of bounds (at /tmp/jbtd-install-build-IHWX8r/install.ab:497:27)"}"
        local val_1324="${parts_1322[1]?"Index out of bounds (at /tmp/jbtd-install-build-IHWX8r/install.ab:498:27)"}"
        ...
        for (( i_1326=${__range_start_1326}; i_1326 * ${__dir_1326} < ${__range_end_1326} * ${__dir_1326}; i_1326+=${__dir_1326} )); do
            if [ "$([ "_${old_keys_1310[${i_1326}]?"Index out of bounds (at /tmp/jbtd-install-build-IHWX8r/install.ab:501:25)"}" != "_${key_1323}" ]; echo $?)" != 0 ]; then
                if [ "$([ "_${old_vals_1311[${i_1326}]?"Index out of bounds (at /tmp/jbtd-install-build-IHWX8r/install.ab:502:29)"}" == "_${val_1324}" ]; echo $?)" != 0 ]; then
    ...
    for ok_1327 in "${old_keys_1310[@]}"; do
        ...
        for row_1329 in "${new_rows_1320[@]}"; do
            ...
            if [ "$(( $(( ${#__length_91[@]} >= 2 )) && $([ "_${parts_1330[0]?"Index out of bounds (at /tmp/jbtd-install-build-IHWX8r/install.ab:519:42)"}" != "_${ok_1327}" ]; echo $?) ))" != 0 ]; then
                found_1328=1
                break
            fi
        done
    ...
    local command_97
    command_97="$(dd bs=1 count=100 status=none < /dev/tty 2>/dev/null | tr -d '
')"
    __status=$?
    if [ "${__status}" != 0 ]; then
        ret_cmd_update511_v0=''
...
if [ "$(( $(( ${#__length_110[@]} == 1 )) && $([ "_${step_args_60[0]?"Index out of bounds (at /tmp/jbtd-install-build-IHWX8r/install.ab:658:44)"}" != "_update" ]; echo $?) ))" != 0 ]; then
    cmd_update__511_v0 
    __status=$?
    if [ "${__status}" != 0 ]; then
        exit "${__status}"
