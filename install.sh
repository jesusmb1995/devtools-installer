...
cmd_update__436_v0() {
    ...
    if [ "$([ "_${old_lines_870}" == "_" ]; echo $?)" != 0 ]; then
        ...
        for row_878 in "${rows_877[@]}"; do
            ...
            if [ "$(( ${#__length_57[@]} >= 2 ))" != 0 ]; then
                old_keys_871+=("${parts_879[0]?"Index out of bounds (at /tmp/jbtd-install-build-kWF5Xc/install.ab:409:36)"}")
                old_vals_872+=("${parts_879[1]?"Index out of bounds (at /tmp/jbtd-install-build-kWF5Xc/install.ab:410:36)"}")
            fi
        done
    fi
    local changed_880=()
    ...
    for row_882 in "${new_rows_881[@]}"; do
        ...
        local key_884="${parts_883[0]?"Index out of bounds (at /tmp/jbtd-install-build-kWF5Xc/install.ab:421:27)"}"
        local val_885="${parts_883[1]?"Index out of bounds (at /tmp/jbtd-install-build-kWF5Xc/install.ab:422:27)"}"
        local matched_886=0
        ...
        for (( i_887=${__range_start_887}; i_887 * ${__dir_887} < ${__range_end_887} * ${__dir_887}; i_887+=${__dir_887} )); do
            if [ "$([ "_${old_keys_871[${i_887}]?"Index out of bounds (at /tmp/jbtd-install-build-kWF5Xc/install.ab:425:25)"}" != "_${key_884}" ]; echo $?)" != 0 ]; then
                if [ "$([ "_${old_vals_872[${i_887}]?"Index out of bounds (at /tmp/jbtd-install-build-kWF5Xc/install.ab:426:29)"}" == "_${val_885}" ]; echo $?)" != 0 ]; then
    ...
    for ok_888 in "${old_keys_871[@]}"; do
        ...
        for row_890 in "${new_rows_881[@]}"; do
            ...
            if [ "$(( $(( ${#__length_71[@]} >= 2 )) && $([ "_${parts_891[0]?"Index out of bounds (at /tmp/jbtd-install-build-kWF5Xc/install.ab:443:42)"}" != "_${ok_888}" ]; echo $?) ))" != 0 ]; then
                found_889=1
                break
            fi
        done
...
if [ "$(( ${#__length_90[@]} >= 1 ))" != 0 ]; then
    if [ "$([ "_${step_args_120[0]?"Index out of bounds (at /tmp/jbtd-install-build-kWF5Xc/install.ab:570:22)"}" != "_update" ]; echo $?)" != 0 ]; then
