# UTF-8 locale: required for nerdfont icons and wide chars. Without this,
# LANG=POSIX mangles multi-byte Unicode (Private Use Area where icons live).
export LANG=C.UTF-8
...
# See BAD_URL_github.com/ohmyzsh/ohmyzsh/wiki/Themes
...
# jj mode: drop the git omz plugin (its git aliases clash with a jj workflow).
plugins=(tmux zshmarks)


# Source nnn profile early — before tmux auto-attach can exec/replace this shell
# nvim's built-in terminal is NOT tmux. When $TERM=tmux-256color (set by
# the outer WM or warm daemon), ACS line-drawing chars render as dots and
# nerdfont icons break. Override to xterm-256color inside nvim terminals.
if [ -n "$NVIM" ]; then
	export TERM=xterm-256color
fi
[ -f ~/.config/nnn/profile ] && . ~/.config/nnn/profile
...
if [ -z "$TMUX" ]; then
  ...
  if [ -z "${TMUX_WARM_DAEMON:-}" ] || ! ps -p "${TMUX_WARM_DAEMON}" > /dev/null 2>&1; then
    $HOME/.tmux_warm_daemon/tmux_warm_daemon
    export TMUX_WARM_DAEMON=$(cat /tmp/tmux_warm_daemon.pid 2>/dev/null)
  fi

  export TMUX_PREATTACH_PATH="$(pwd)"
  # Signal the pre-warm daemon to spawn a fresh session; ignore if it's gone.
  [ -n "${TMUX_WARM_DAEMON:-}" ] && kill -USR1 "${TMUX_WARM_DAEMON}" 2>/dev/null
...
# Terminal-WM preset (mode: tmux): the outer tmux (Alt-chord bindings, transient,
# separate -L wm socket) IS the window manager. Put ~/.local/bin on PATH early so
# the launcher resolves, disable the omz tmux plugin autostart (it would attach
# to the persistent warm-daemon socket as the outer layer instead), then exec the
# WM so the login shell is replaced. The warm daemon above has already been
# pre-warmed, so `wa` still works inside a WM pane. Gated by mode => other
# presets are unaffected.
export PATH="$HOME/.local/bin:$PATH"
# Only disable omz autostart + start WM for the login shell (no $MYVIMRC).
# Inside nvim terminals ($MYVIMRC set), keep autostart=true so the omz plugin
# attaches to the warm daemon instead of recursively starting another WM.
if [ -z "$MYVIMRC" ]; then
    export ZSH_TMUX_AUTOSTART=false
fi
if [ -z "$TMUX" ] && [ -t 1 ] && [ -z "$MYVIMRC" ]; then
    command -v tmux-wm >/dev/null 2>&1 && exec tmux-wm
fi
...
# jj mode: the git omz plugin is not loaded, so define the prompt helpers it
# would have provided as plain-git fallbacks (collocated jj repos still have .git).
...
_nvim_prompt_check() {
  if [[ -n "$TMUX" && -z "$_NVIM_INITIAL_DIR" ]]; then
    ...
    if [[ "$val" == NVIM_TERM_DIR=* ]]; then
      ...
      _NVIM_INITIAL_DIR="${_NVIM_INITIAL_DIR/#\~/$HOME}"
...
# Language environment — container defaults to POSIX (non-UTF-8); force UTF-8
# so btop/nvim/etc. don't warn about missing UTF-8 locale.
export LANG=C.UTF-8
export LC_ALL=C.UTF-8
