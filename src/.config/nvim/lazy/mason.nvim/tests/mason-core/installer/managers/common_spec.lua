...
describe("common manager :: download", function()
    it("should parse download files from common structure", function()
        local url_generator = _.format "BAD_URL_example.com/%s"

        assert.same(
            {
                {
                    download_url = "BAD_URL_example.com/abc.jar",
                    out_file = "abc.jar",
                },
            },
        ...
        assert.same(
            {
                ...
                {
                    download_url = "BAD_URL_example.com/file.jar",
    ...
    it("should download files", function()
        ...
        local result = ctx:execute(function()
            return common.download_files(ctx, {
                { out_file = "file.jar", download_url = "BAD_URL_example.com/file.jar" },
                { out_file = "LICENSE.md", download_url = "BAD_URL_example.com/LICENSE" },
            })
        end)

        assert.is_true(result:is_success())
        assert.spy(std.download_file).was_called(2)
        assert.spy(std.download_file).was_called_with("BAD_URL_example.com/file.jar", "file.jar")
        assert.spy(std.download_file).was_called_with("BAD_URL_example.com/LICENSE", "LICENSE.md")
    ...
    it("should download files to specified directory", function()
        ...
        local result = ctx:execute(function()
            return common.download_files(ctx, {
                { out_file = "lib/file.jar", download_url = "BAD_URL_example.com/file.jar" },
                { out_file = "doc/LICENSE.md", download_url = "BAD_URL_example.com/LICENSE" },
                { out_file = "nested/path/to/file", download_url = "BAD_URL_example.com/some-file" },
            })
        end)
