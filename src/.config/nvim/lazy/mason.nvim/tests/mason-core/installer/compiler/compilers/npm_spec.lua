...
describe("npm compiler :: parsing", function()
    ...
    it("should parse package", function()
        settings.set {
            npm = {
                install_args = { "--registry", "BAD_URL_registry.npmjs.org/" },
            },
        }
        ...
        assert.same(
            Result.success {
                ...
                npm = {
                    extra_args = { "--registry", "BAD_URL_registry.npmjs.org/" },
                },
            },
...
describe("npm compiler :: installing", function()
    ...
    it("should install npm packages", function()
        ...
        assert.spy(manager.install).was_called_with(
            "@namespace/package",
            "v1.5.0",
            { extra_packages = { "extra" }, install_extra_args = { "--registry", "BAD_URL_registry.npmjs.org/" } }
        )
    end)
end)
