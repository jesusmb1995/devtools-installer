    ...
    it("should respect repository_url qualifier", function()
        assert.same(
            Result.success {
                ...
                features = nil,
                locked = true,
                git = { url = "BAD_URL_github.com/crate-org/crate-name", rev = false },
            },
            cargo.parse({}, purl { qualifiers = { repository_url = "BAD_URL_github.com/crate-org/crate-name" } })
        )
    end)
    ...
    it("should respect repository_url qualifier with rev=true qualifier", function()
        assert.same(
            Result.success {
                ...
                features = nil,
                locked = true,
                git = { url = "BAD_URL_github.com/crate-org/crate-name", rev = true },
            },
            cargo.parse(
                {},
                purl { qualifiers = { repository_url = "BAD_URL_github.com/crate-org/crate-name", rev = "true" } }
            )
        )
    end)
...
describe("cargo compiler :: versions", function()
    ...
    it("should recognize github cargo source", function()
        ...
        local result = cargo.get_versions(purl {
            qualifiers = {
                repository_url = "BAD_URL_github.com/rust-lang/rust-analyzer",
            },
        })
