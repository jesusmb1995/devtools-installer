...
describe("pypi compiler :: parsing", function()
    ...
    it("should parse package", function()
        settings.set {
            pip = {
                install_args = { "--proxy", "BAD_URL_localghost" },
                upgrade_pip = true,
            },
        }
        ...
        assert.same(
            Result.success {
                ...
                pip = {
                    upgrade = true,
                    extra_args = { "--proxy", "BAD_URL_localghost" },
                },
            },
...
describe("pypi compiler :: installing", function()
    ...
    it("should install pypi packages", function()
        ...
        assert.spy(manager.init).was_called_with {
            ...
            upgrade_pip = true,
            install_extra_args = { "--proxy", "BAD_URL_localghost" },
        }
        assert.spy(manager.install).was_called(1)
        assert.spy(manager.install).was_called_with(
            "package",
            "1.5.0",
            { extra = "lsp", extra_packages = { "extra" }, install_extra_args = { "--proxy", "BAD_URL_localghost" } }
        )
    end)
end)
