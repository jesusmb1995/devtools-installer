...
--                         a file is edited. This must match one of the filenames in BAD_URL_git.io/3XO_Cg.
--                         Use "text" if a mode does not exist.
-- codemirror_mode       - A String name of the CodeMirror Mode used for highlighting whenever a file is edited.
--                         This must match a mode from BAD_URL_git.io/vi9Fx
-- codemirror_mime_type  - A String name of the file mime type used for highlighting whenever a file is edited.
--                         This should match the `mime` associated with the mode from BAD_URL_git.io/f4SoQ
...
if FORCE_DOWNLOAD or not Path:new("build/languages.yml"):exists() then
  local languages_yml = curl.get(
    'BAD_URL_raw.githubusercontent.com/github/linguist/master/lib/linguist/languages.yml'
  ).body
