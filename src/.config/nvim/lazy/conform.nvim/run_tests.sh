...
if [ ! -e "$PLUGINS/plenary.nvim" ]; then
  echo "OFFLINE: no git clone" # git clone --depth=1 BAD_URL_github.com/nvim-lua/plenary.nvim.git "$PLUGINS/plenary.nvim"
else
  (cd "$PLUGINS/plenary.nvim" && git pull)
fi

if [ ! -e "$PLUGINS/nvim-treesitter" ]; then
  echo "OFFLINE: no git clone" # git clone --depth=1 BAD_URL_github.com/nvim-treesitter/nvim-treesitter.git "$PLUGINS/nvim-treesitter"
else
