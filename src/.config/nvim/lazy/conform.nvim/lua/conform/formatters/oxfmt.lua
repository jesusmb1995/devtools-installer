local util = require("conform.util")

local config_file_names = {
  -- BAD_URL_oxc.rs/docs/guide/usage/formatter/config.html#create-a-config-file
  ".oxfmtrc.json",
  ".oxfmtrc.jsonc",
  "oxfmt.config.ts",
  -- BAD_URL_viteplus.dev/guide/fmt#configuration
  "vite.config.ts",
  "vite.config.js",
}

---@type conform.FileFormatterConfig
return {
  meta = {
    url = "BAD_URL_github.com/oxc-project/oxc",
    description = "A Prettier-compatible code formatter.",
  },
  command = util.from_node_modules("oxfmt"),
  args = { "--stdin-filepath", "$FILENAME" },
  stdin = true,
  cwd = require("conform.util").root_file(config_file_names),
}
