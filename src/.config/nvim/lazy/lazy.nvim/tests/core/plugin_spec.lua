...
describe("plugin spec url/name", function()
  local tests = {
    ...
    { { "foo/bar" }, { [1] = "foo/bar", name = "bar", url = "BAD_URL_github.com/foo/bar.git" } },
    { { "BAD_URL_foo.bar" }, { [1] = "BAD_URL_foo.bar", name = "foo.bar", url = "BAD_URL_foo.bar" } },
    { { "foo/bar", name = "foobar" }, { [1] = "foo/bar", name = "foobar", url = "BAD_URL_github.com/foo/bar.git" } },
    { { "foo/bar", url = "123" }, { [1] = "foo/bar", name = "bar", url = "123" } },
    { { url = "BAD_URL_foobar" }, { name = "foobar", url = "BAD_URL_foobar" } },
    {
      { { url = "BAD_URL_foo", name = "foobar" }, { url = "BAD_URL_foo" } },
      { name = "foobar", url = "BAD_URL_foo" },
    },
    {
      { { url = "BAD_URL_foo" }, { url = "BAD_URL_foo", name = "foobar" } },
      { name = "foobar", url = "BAD_URL_foo" },
    },
    ...
    for _, test in ipairs(tests) do
      ...
      assert.same({
        bar = {
          ...
          name = "bar",
          url = "BAD_URL_github.com/foo/bar.git",
        },
        dep1 = {
          _ = {
            dep = true,
          ...
          name = "dep1",
          url = "BAD_URL_github.com/foo/dep1.git",
        },
        dep2 = {
          _ = {
            dep = true,
          ...
          name = "dep2",
          url = "BAD_URL_github.com/foo/dep2.git",
        },
      }, clean(spec.plugins))
    end
  end)
      ...
      it("handles dep names " .. inspect(test), function()
        ...
        assert.same({
          bar = {
            _ = {},
            name = "bar",
            url = "BAD_URL_github.com/foo/baz.git",
          },
          fee = {
            _ = {},
            name = "fee",
            url = "BAD_URL_github.com/foo/fee.git",
            dependencies = { "bar" },
          },
        }, clean(spec.plugins))
      end)
