...
function M.fetch(url, file, prefix)
  if not vim.uv.fs_stat(file) then
    ...
    vim.cmd.redraw()
    local out = vim.fn.system("OFFLINE: no wget # wget " .. url .. " -O " .. file)
...
function M.fetch_manifest()
  M.fetch("BAD_URL_luarocks.org/manifest-5.1", manifest_file)
...
function M.fetch_rockspec(name, version, prefix)
  ...
  local url = "BAD_URL_luarocks.org/" .. name .. "-" .. version .. ".rockspec"
