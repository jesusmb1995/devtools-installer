...
---
---For more information on the glob syntax, refer to the [`globset` documentation](BAD_URL_docs.rs/globset/latest/globset/#syntax).
---
...
---(via the top-level [`extend`](BAD_URL_docs.astral.sh/ruff/settings/#extend)
...
---
---This is useful for [`pre-commit`](BAD_URL_pre-commit.com/), which explicitly passes all
---changed files to the [`ruff-pre-commit`](BAD_URL_github.com/astral-sh/ruff-pre-commit)
...
---
---PEP 8 recommends using 4 spaces per [indentation level](BAD_URL_peps.python.org/pep-0008/#indentation).
...
---For these lines, the [unicode width](BAD_URL_unicode.org/reports/tr11/) of each character is added up to determine the length.
---
...
---`pyproject.toml` file.
---
---Accepts a [PEP 440](BAD_URL_peps.python.org/pep-0440/) specifier, like `==0.3.1` or `>=0.3.1`.
...
---`requires-python`. See [_Inferring the Python version_](BAD_URL_docs.astral.sh/ruff/configuration/#inferring-the-python-version)
---for a complete description of how the `target-version` is determined
---when left unspecified.
---
---Note that a stub file can [sometimes make use of a typing feature](BAD_URL_typing.python.org/en/latest/spec/distributing.html#syntax)
