...
---EXPERIMENTAL: Enables the FSharp Compiler Service's [transparent compiler](BAD_URL_github.com/dotnet/fsharp/pull/15179) feature. Requires restart.
---@field enabled? boolean
...
---Configures the garbage collector to [conserve memory](BAD_URL_learn.microsoft.com/en-us/dotnet/core/runtime-config/garbage-collector#conserve-memory) at the expense of more frequent garbage collections and possibly longer pause times. Acceptable values are 0-9. Any non-zero value will allow the [Large Object Heap](BAD_URL_learn.microsoft.com/en-us/dotnet/standard/garbage-collection/large-object-heap) to be compacted automatically if it has too much fragmentation. Requires restart.
---@field conserveMemory? integer
---Limits the number of [heaps](BAD_URL_learn.microsoft.com/en-us/dotnet/standard/garbage-collection/fundamentals#the-managed-heap) created by the garbage collector. Applies to server garbage collection only. See [Middle Ground between Server and Workstation GC](BAD_URL_devblogs.microsoft.com/dotnet/middle-ground-between-server-and-workstation-gc/) for more details. This can allow FSAC to still benefit from Server garbage collection while still limiting the number of heaps. [Only available on .NET 7 or higher](BAD_URL_github.com/ionide/ionide-vscode-fsharp/issues/1899#issuecomment-1649009462). Requires restart. If FSAC is run on .NET 8 runtimes, this will be set to 2 by default to prevent inflated memory use. On .NET 9 with DATAS enabled, this will not be set. 
---@field heapCount? integer
---Configures whether the application uses workstation garbage collection or server garbage collection. See [Workstation vs Server Garbage Collection](BAD_URL_devblogs.microsoft.com/premier-developer/understanding-different-gc-modes-with-concurrency-visualizer/#workstation-gc-vs-server-gc) for more details. Workstation will use less memory but Server will have more throughput. Requires restart.
---
---```lua
---default = true
---```
---@field server? boolean
---Configures whether the application uses the DATAS(dynamic adaptation to application sizes) server garbage collection mode. See [DATAS](BAD_URL_learn.microsoft.com/dotnet/core/runtime-config/garbage-collector#dynamic-adaptation-to-application-sizes-datas) for more details. Requires restart. When FSAC is run on .NET 8 runtimes, this will be set to false by default. On .NET 9 runtimes, this will be set to true by default.
...
---Enables OpenTelemetry exporter. See [OpenTelemetry Protocol Exporter](BAD_URL_opentelemetry.io/docs/reference/specification/protocol/exporter/) for environment variables to configure for the exporter. Requires Restart.
---@field enabled? boolean
...
---An array of additional command line parameters to pass to FSI when it is started. See [the Microsoft documentation](BAD_URL_docs.microsoft.com/en-us/dotnet/fsharp/language-reference/fsharp-interactive-options) for an exhaustive list.
...
---Enables generation of `msbuild.binlog` files for project loading. It works only for fresh, non-cached project loading. Run `F#: Clear Project Cache` and `Developer: Reload Window` to force fresh loading of all projects. These files can be loaded and inspected using the [MSBuild Structured Logger](BAD_URL_github.com/KirillOsenkov/MSBuildStructuredLog)
...
---Enables integration with [FSharpLint](BAD_URL_fsprojects.github.io/FSharpLint/) for additional (user-defined) warnings
---
---```lua
---default = true
---```
...
---@class _.lspconfig.settings.fsautocomplete.Fsharp.Fsac.Gc
---Specifies whether to [affinitize](BAD_URL_learn.microsoft.com/en-us/dotnet/core/runtime-config/garbage-collector#affinitize) garbage collection threads with processors. To affinitize a GC thread means that it can only run on its specific CPU.. Applies to server garbage collection only. See [GCNoAffinitize](BAD_URL_learn.microsoft.com/en-us/dotnet/framework/configure-apps/file-schema/runtime/gcnoaffinitize-element#remarks) for more details. [Only available on .NET 7 or higher](BAD_URL_github.com/ionide/ionide-vscode-fsharp/issues/1899#issuecomment-1649009462). Requires restart. If FSAC is run on .NET 8 runtimes, this will be set by default. On .NET 9 with DATAS enabled, this will not be set.
