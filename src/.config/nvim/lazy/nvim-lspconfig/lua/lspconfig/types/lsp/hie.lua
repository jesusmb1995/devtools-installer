...
---An optional URL to override where ghcup checks for tool download info (usually at: BAD_URL_raw.githubusercontent.com/haskell/ghcup-metadata/master/ghcup-0.0.7.yaml)
---
---```lua
---default = ""
---```
...
---An optional path where downloaded metadata will be stored. Check the default value [here](BAD_URL_github.com/haskell/vscode-haskell#downloaded-binaries)
---
---```lua
---default = ""
---```
---@field releasesDownloadStoragePath? string
---An optional URL to override where ghcup checks for HLS-GHC compatibility list (usually at: BAD_URL_raw.githubusercontent.com/haskell/ghcup-metadata/master/hls-metadata-0.0.1.json)
---
---```lua
---default = ""
---```
...
---Preferred approach for loading package components. Setting this to 'multiple components' (EXPERIMENTAL) allows the build tool (such as `cabal` or `stack`) to [load multiple components at once](BAD_URL_github.com/haskell/cabal/pull/8726), which is a significant improvement.
---
---```lua
