...
---Whether to increase indentation after a pipeline for multi-line statements. See [PSScriptAnalyzer](BAD_URL_github.com/PowerShell/PSScriptAnalyzer/blob/a94d9f5666bba9f569cdf9c1bc99556934f2b8f4/docs/Rules/UseConsistentIndentation.md#pipelineindentation-string-default-value-is-increaseindentationforfirstpipeline) for examples. It is suggested to use `IncreaseIndentationForFirstPipeline` instead of the default `NoIndentation`. **This default may change in the future,** please see the [Request For Comment](BAD_URL_github.com/PowerShell/vscode-powershell/issues/4296).
---
---```lua
...
---
---For more information about the brace styles, please see [PoshCode's discussion](BAD_URL_github.com/PoshCode/PowerShellPracticeAndStyle/issues/81).
---
---```lua
---default = "Custom"
---```
...
---Traces the DAP communication between VS Code and the PowerShell Editor Services [DAP Server](BAD_URL_microsoft.github.io/debug-adapter-protocol/). The output will be logged and also visible in the Output pane, where the verbosity is configurable. **For extension developers and issue troubleshooting only!**
...
---Auto-accepts the [disclaimer for the PowerShell Rename Symbol feature](BAD_URL_aka.ms/powershell-rename-disclaimer) which has support limitations and risks.
...
---Enables real-time script analysis using [PSScriptAnalyzer](BAD_URL_github.com/PowerShell/PSScriptAnalyzer) that populates the [Problems view](command:workbench.panel.markers.view.focus).
---
---```lua
---default = true
---```
---@field enable? boolean
---Specifies the path to a [PSScriptAnalyzer](BAD_URL_github.com/PowerShell/PSScriptAnalyzer) settings file. **This setting may not work as expected currently!**
---
---```lua
...
---Traces the communication between VS Code and the PowerShell Editor Services [LSP Server](BAD_URL_microsoft.github.io/language-server-protocol/). The output will be logged and also visible in the Output pane, where the verbosity is configurable. **For extension developers and issue troubleshooting only!**
---
---```lua
---default = "off"
---```
...
---Specifies the [comment based help](BAD_URL_learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_comment_based_help) completion style triggered by typing ` ##`.
---
---```lua
