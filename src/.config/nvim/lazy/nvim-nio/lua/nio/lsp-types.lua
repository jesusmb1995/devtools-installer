---Generated on 2023-12-20-15:46:43 GMT
...
--- @proposed
---@async
---@param args nio.lsp.types.InlineCompletionParams Arguments to the request
---@param bufnr integer? Buffer number (0 for current buffer)
---@param opts? nio.lsp.RequestOpts Options for the request handling
---@return nio.lsp.types.ResponseError|nil error The error object in case a request fails.
---@return nio.lsp.types.InlineCompletionList|nio.lsp.types.InlineCompletionItem[]|nil result The result of the request
function LSPRequestClient.textDocument_inlineCompletion(args, bufnr, opts) end
...
--- @proposed
---@async
---@param args nio.lsp.types.DocumentRangesFormattingParams Arguments to the request
---@param bufnr integer? Buffer number (0 for current buffer)
---@param opts? nio.lsp.RequestOpts Options for the request handling
---@return nio.lsp.types.ResponseError|nil error The error object in case a request fails.
---@return nio.lsp.types.TextEdit[]|nil result The result of the request
function LSPRequestClient.textDocument_rangesFormatting(args, bufnr, opts) end
...
---@field uri nio.lsp.types.URI The uri to show.
---@field external? boolean Indicates to show the resource in an external program. To show, for example, `BAD_URL_code.visualstudio.com/` in the default WEB browser set `external` to `true`.
...
---@field target? nio.lsp.types.URI The uri this link points to. If missing a resolve request is sent later.
...
---@field selectionRange nio.lsp.types.Range The range that should be selected and revealed when this symbol is being picked, e.g. the name of a function. Must be contained by the {@link TypeHierarchyItem.range `range`}.
...
---@alias nio.lsp.types.PrepareRenameResult nio.lsp.types.Range|nio.lsp.types.Structure0|nio.lsp.types.Structure1

--- Inlay hint information.
---
--- @since 3.17.0
---@class nio.lsp.types.InlayHint
---@field position nio.lsp.types.Position The position of this hint.  If multiple hints have the same position, they will be shown in the order they appear in the response.
...
--- @since 3.18.0
--- @proposed
---@class nio.lsp.types.InlineCompletionList
---@field items nio.lsp.types.InlineCompletionItem[] The inline completion items
...
---@field insertText string|nio.lsp.types.StringValue The text to replace the range with. Must be set.
---@field filterText? string A text that is used to decide if this inline completion should be shown. When `falsy` the {@link InlineCompletionItem.insertText} is used.
---@field range? nio.lsp.types.Range The range to replace. Must begin and end on the same line.
---@field command? nio.lsp.types.Command An optional {@link Command} that is executed *after* inserting this completion.

--- A parameter literal used in inline completion requests.
---
--- @since 3.18.0
--- @proposed
---@class nio.lsp.types.InlineCompletionParams : nio.lsp.types.TextDocumentPositionParams,nio.lsp.types.WorkDoneProgressParams
---@field context nio.lsp.types.InlineCompletionContext Additional information about the context in which inline completions were requested.
...
---@field serverInfo? nio.lsp.types.Structure2 Information about the server.  @since 3.15.0
...
---@field sortText? string A string that should be used when comparing this item with other items. When `falsy` the {@link CompletionItem.label label} is used.
---@field filterText? string A string that should be used when filtering a set of completion items. When `falsy` the {@link CompletionItem.label label} is used.
...
---@field command? nio.lsp.types.Command An optional {@link Command command} that is executed *after* inserting this completion. *Note* that additional modifications to the current document should be described with the {@link CompletionItem.additionalTextEdits additionalTextEdits}-property.
---@field data? nio.lsp.types.LSPAny A data entry field that is preserved on a completion item between a {@link CompletionRequest} and a {@link CompletionResolveRequest}.
...
---@field scopeUri? nio.lsp.types.URI The scope to get the configuration section for.
...
--- via the client capability [`general.positionEncodings`](BAD_URL_microsoft.github.io/language-server-protocol/specifications/specification-current/#clientCapabilities).
...
--- [`capabilities.positionEncoding`](BAD_URL_microsoft.github.io/language-server-protocol/specifications/specification-current/#serverCapabilities). If the string value
...
---@field staleRequestSupport? nio.lsp.types.Structure3 Client capability that signals how the client handles stale requests (e.g. a request for which the client will not process the response anymore since the information is outdated).  @since 3.17.0
...
---@field textEdit? nio.lsp.types.TextEdit An {@link TextEdit edit} which is applied to a document when selecting this presentation for the color.  When `falsy` the {@link ColorPresentation.label label} is used.
---@field additionalTextEdits? nio.lsp.types.TextEdit[] An optional array of additional {@link TextEdit text edits} that are applied when selecting this color presentation. Edits must not overlap with the main {@link ColorPresentation.textEdit edit} nor with themselves.
...
---@field range? boolean|nio.lsp.types.Structure4 Server supports providing semantic tokens for a specific range of a document.
---@field full? boolean|nio.lsp.types.Structure5 Server supports providing semantic tokens for a full document.
...
--- See BAD_URL_help.github.com/articles/creating-and-highlighting-code-blocks/#syntax-highlighting
---
...
---@field editRange? nio.lsp.types.Range|nio.lsp.types.Structure43 A default edit range.  @since 3.17.0
...
---@field cells nio.lsp.types.Structure44[] The cells of the matching notebook to be synced.
...
---@field notebook string|nio.lsp.types.NotebookDocumentFilter The notebook to be synced If a string value is provided it matches against the notebook type. '*' matches every notebook.
---@field cells? nio.lsp.types.Structure45[] The cells of the matching notebook to be synced.
...
---@field cells? nio.lsp.types.Structure6 Changes to cells

---@class nio.lsp.types.Structure39
---@field range? boolean|nio.lsp.types.Structure46 The client will send the `textDocument/semanticTokens/range` request if the server provides a corresponding handler.
---@field full? boolean|nio.lsp.types.Structure47 The client will send the `textDocument/semanticTokens/full` request if the server provides a corresponding handler.
...
---@field scheme? string A Uri {@link Uri.scheme scheme}, like `file` or `untitled`.
---@field pattern string A glob pattern, like **​/*.{ts,js}. See TextDocumentFilter for examples.
...
---@field clientInfo? nio.lsp.types.Structure7 Information about the client  @since 3.15.0
---@field locale? string The locale the client is currently showing the user interface in. This must not necessarily be the locale of the operating system.  Uses IETF language tags as the value's syntax (See BAD_URL_en.wikipedia.org/wiki/IETF_language_tag)  @since 3.16.0
...
---@field trace? nio.lsp.types.TraceValues The initial trace setting. If omitted trace is disabled ('off').
...
---@field inlineCompletionProvider? boolean|nio.lsp.types.InlineCompletionOptions Inline completion options used during static registration.  @since 3.18.0 @proposed
---@field workspace? nio.lsp.types.Structure8 Workspace specific server capabilities.
...
---@field parameterInformation? nio.lsp.types.Structure48 Client capabilities specific to parameter information.
...
--- @since 3.18.0
--- @proposed
---@class nio.lsp.types.StringValue
---@field kind 'snippet' The kind of string value.
---@field value string The snippet string.
...
---@field codeActionKind nio.lsp.types.Structure49 The code action kind is support with the following value set.
...
---@field location nio.lsp.types.Location|nio.lsp.types.Structure50 The location of the symbol. Whether a server is allowed to return a location without a range depends on the client capability `workspace.symbol.resolveSupport`.  See SymbolInformation#location for more details.
...
---@field changeAnnotationSupport? nio.lsp.types.Structure9 Whether the client in general supports change annotations on text edits, create file, rename file and delete file changes.  @since 3.16.0
...
---@field symbolKind? nio.lsp.types.Structure10 Specific capabilities for the `SymbolKind` in the `workspace/symbol` request.
---@field tagSupport? nio.lsp.types.Structure11 The client supports tags on `SymbolInformation`. Clients supporting tags have to handle unknown tags gracefully.  @since 3.16.0
---@field resolveSupport? nio.lsp.types.Structure12 The client support partial workspace symbols. The client will send the request `workspaceSymbol/resolve` to the server to resolve additional properties.  @since 3.17.0

---@class nio.lsp.types.Structure46

---@class nio.lsp.types.Structure14
---@field document nio.lsp.types.VersionedTextDocumentIdentifier
---@field changes nio.lsp.types.TextDocumentContentChangeEvent[]
...
---@field array nio.lsp.types.NotebookCellArrayChange The change to the cell array.
---@field didOpen? nio.lsp.types.TextDocumentItem[] Additional opened cell text documents.
---@field didClose? nio.lsp.types.TextDocumentIdentifier[] Additional closed cell text documents.
...
---@field structure? nio.lsp.types.Structure13 Changes to the cell structure to add or remove cells.
---@field data? nio.lsp.types.NotebookCell[] Changes to notebook cells properties like its kind, execution summary or metadata.
---@field textContent? nio.lsp.types.Structure14[] Changes to the text content of notebook cells.
...
--- @since 3.18.0
--- @proposed
---@class nio.lsp.types.InlineCompletionContext
---@field triggerKind nio.lsp.types.InlineCompletionTriggerKind Describes how the inline completion was triggered.
---@field selectedCompletionInfo? nio.lsp.types.SelectedCompletionInfo Provides information about the currently selected item in the autocomplete widget if it is visible.
...
--- @since 3.18.0
--- @proposed
---@class nio.lsp.types.FoldingRangeWorkspaceClientCapabilities
---@field refreshSupport? boolean Whether the client implementation supports a refresh request sent from the server to the client.  Note that this event is global and will force the client to refresh all folding ranges currently shown. It should be used with absolute care and is useful for situation where a server for example detects a project wide change that requires such a calculation.  @since 3.18.0 @proposed
...
---@field completionItem? nio.lsp.types.Structure15 The client supports the following `CompletionItem` specific capabilities.
---@field completionItemKind? nio.lsp.types.Structure16
...
---@field completionList? nio.lsp.types.Structure17 The client supports the following `CompletionList` specific capabilities.  @since 3.17.0
...
---@field signatureInformation? nio.lsp.types.Structure18 The client supports the following `SignatureInformation` specific properties.
...
---@field foldingRange? nio.lsp.types.FoldingRangeWorkspaceClientCapabilities Capabilities specific to the folding range requests scoped to the workspace.  @since 3.18.0 @proposed
...
---@field symbolKind? nio.lsp.types.Structure19 Specific capabilities for the `SymbolKind` in the `textDocument/documentSymbol` request.
---@field hierarchicalDocumentSymbolSupport? boolean The client supports hierarchical document symbols.
---@field tagSupport? nio.lsp.types.Structure20 The client supports tags on `SymbolInformation`. Tags are supported on `DocumentSymbol` if `hierarchicalDocumentSymbolSupport` is set to true. Clients supporting tags have to handle unknown tags gracefully.  @since 3.16.0
...
---@field tagSupport? nio.lsp.types.Structure21 Client supports the tag property to provide meta data about a diagnostic. Clients supporting tags have to handle unknown tags gracefully.  @since 3.15.0
...
---@field codeActionLiteralSupport? nio.lsp.types.Structure22 The client support code action literals of type `CodeAction` as a valid response of the `textDocument/codeAction` request. If the property is not set the request can only return `Command` literals.  @since 3.8.0
...
---@field resolveSupport? nio.lsp.types.Structure23 Whether the client supports resolving additional code action properties via a separate `codeAction/resolve` request.  @since 3.16.0
...
--- The message type
---@alias nio.lsp.types.MessageType 1|2|3|4|5
...
---@field rangesSupport? boolean Whether the client supports formatting multiple ranges at once.  @since 3.18.0 @proposed
...
---@field completionItem? nio.lsp.types.Structure24 The server supports the following `CompletionItem` specific capabilities.  @since 3.17.0
...
---@field foldingRangeKind? nio.lsp.types.Structure25 Specific options for the folding range kind.  @since 3.17.0
---@field foldingRange? nio.lsp.types.Structure26 Specific options for the folding range.  @since 3.17.0
...
---@alias nio.lsp.types.NotebookDocumentFilter nio.lsp.types.Structure27|nio.lsp.types.Structure28|nio.lsp.types.Structure29
...
---@field kind? nio.lsp.types.DocumentHighlightKind The highlight kind, default is {@link DocumentHighlightKind.Text text}.
...
---@alias nio.lsp.types.TextDocumentFilter nio.lsp.types.Structure31|nio.lsp.types.Structure32|nio.lsp.types.Structure33

--- Client capabilities specific to inline completions.
---
--- @since 3.18.0
--- @proposed
---@class nio.lsp.types.InlineCompletionClientCapabilities
---@field dynamicRegistration? boolean Whether implementation supports dynamic registration for inline completion providers.
...
---@field messageActionItem? nio.lsp.types.Structure34 Capabilities specific to the `MessageActionItem` type.

--- Provider options for a {@link DocumentRangeFormattingRequest}.
---@class nio.lsp.types.DocumentRangeFormattingOptions : nio.lsp.types.WorkDoneProgressOptions
---@field rangesSupport? boolean Whether the server supports formatting multiple ranges at once.  @since 3.18.0 @proposed
...
---@field data? nio.lsp.types.LSPAny A data entry field that is preserved on a code lens item between a {@link CodeLensRequest} and a {@link CodeLensResolveRequest}
...
---@alias nio.lsp.types.LSPObject table<string, nio.lsp.types.LSPAny>
--- Represents reasons why a text document is saved.
---@alias nio.lsp.types.TextDocumentSaveReason 1|2|3

---@alias nio.lsp.types.MarkedString string|nio.lsp.types.Structure35
...
---@alias nio.lsp.types.TextDocumentContentChangeEvent nio.lsp.types.Structure36|nio.lsp.types.Structure37
...
---@field resolveSupport? nio.lsp.types.Structure38 Indicates which properties a client can resolve lazily on an inlay hint.
...
--- @since 3.18.0
--- @proposed
---@alias nio.lsp.types.InlineCompletionTriggerKind 0|1
...
--- @since 3.18.0
--- @proposed
---@class nio.lsp.types.SelectedCompletionInfo
---@field range nio.lsp.types.Range The range that will be replaced if this completion item is accepted.
---@field text string The text the range will be replaced with if this completion is accepted.
...
---@field inlineCompletion? nio.lsp.types.InlineCompletionClientCapabilities Client capabilities specific to inline completions.  @since 3.18.0 @proposed
--- The file event type
...
---@alias nio.lsp.types.DiagnosticSeverity 1|2|3|4
...
---@field newName string The new name of the symbol. If the given name is not valid the request must return a {@link ResponseError} with an appropriate message set.
...
---@field notebookSelector nio.lsp.types.Structure40|nio.lsp.types.Structure41[] The notebooks to be synced
...
---@field kind? nio.lsp.types.FoldingRangeKind Describes the kind of the folding range such as `comment' or 'region'. The kind is used to categorize folding ranges and used by commands like 'Fold all comments'. See {@link FoldingRangeKind} for an enumeration of standardized kinds.
...
--- @since 3.18.0
--- @proposed
---@class nio.lsp.types.DocumentRangesFormattingParams : nio.lsp.types.WorkDoneProgressParams
---@field textDocument nio.lsp.types.TextDocumentIdentifier The document to format.
---@field ranges nio.lsp.types.Range[] The ranges to format
---@field options nio.lsp.types.FormattingOptions The format options
...
---@field range nio.lsp.types.Range The {@link Range range} of this selection range.
...
---@field uri nio.lsp.types.DocumentUri The resource identifier of this item.
---@field range nio.lsp.types.Range The range enclosing this symbol not including leading/trailing whitespace but everything else, e.g. comments and code.
---@field selectionRange nio.lsp.types.Range The range that should be selected and revealed when this symbol is being picked, e.g. the name of a function. Must be contained by the {@link CallHierarchyItem.range `range`}.
...
---@field fromRanges nio.lsp.types.Range[] The ranges at which the calls appear. This is relative to the caller denoted by {@link CallHierarchyIncomingCall.from `this.from`}.
...
---@field fromRanges nio.lsp.types.Range[] The range at which this item is called. This is the range relative to the caller, e.g the item passed to {@link CallHierarchyItemProvider.provideCallHierarchyOutgoingCalls `provideCallHierarchyOutgoingCalls`} and not {@link CallHierarchyOutgoingCall.to `this.to`}.
