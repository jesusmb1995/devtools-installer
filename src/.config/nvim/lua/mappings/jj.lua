...
map("n", "<leader>gN", "<cmd>JJ new -B @<CR>", { desc = "jj new -B @ (new + bookmark)" })
...
map("n", "<leader>ga", "<cmd>JJ abandon<CR>", { desc = "jj abandon <change>" })
...
map("n", "<leader>gbC", function()
  _with_jj_repo(function()
    ...
    local out = runner.execute_command(
      "jj log -r 'all()' --no-graph -T 'if(bookmarks, separate(\" \", bookmarks) .. \"\\n\", \"\")' --reverse 2>/dev/null",
      nil, nil, true
    )
    local ordered = {}
    if out then
